...
//     get pizza price
    ...
    btn = '<div class="del" onclick="removeDessert(' + cartSize + ')">-</div>';
    dtotal.innerHTML = 'Total: ₹' + total;
    dcart.innerHTML += '<li>'+ name + ' ' + size +' :  ₹' + price + btn +'</li>';
}
...
function dshoppingCart() {
    ...
    for (let i = 0; i<cartSize; i++){
        btn = '<div class="del" onclick="removeDessert(' + i + ')">-</div>';
        dcart.innerHTML += '<li>'+ orders[i][0] + orders[i][1] + ' ' + ' :  ₹ ' + orders[i][2] + btn +'</li>';
    }
    dtotal.innerHTML =  'Total: ₹' + total;
}
